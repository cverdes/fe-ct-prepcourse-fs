...
function numeroMasGrande(arrayOfNums) {
   ...
   // Tu código:
   return arrayOfNums.reduce((max, item) => (item > max) ? item : max, -Infinity)
}
...
function cuentoElementos(array) {
   ...
   // Tu código:
   return array.reduce((count, num) => (num >  18) ? ++count: count, 0);
}
...
function mesesDelAño(array) {
   ...
   for (mes of array) {
      if (["Enero", "Marzo", "Noviembre"].includes(mes)) {
         mesesBuscados.add(mes)
      }
   }
   if (mesesBuscados.size === 3) {
      return Array.from(mesesBuscados);
   } else {
